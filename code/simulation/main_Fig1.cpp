//13/01/2021 CP: main code for the brownian bug model
//23/01/2020 CP: This code specifically uses the parameters for the Fig1 in this paper
...
extern const double Utot=0.0; //advection, corresponds to U\tau/2 in Young et al. 2001
...
//Compute the pair density function (reminder: pair dens=pair correlation function \times C^2).
...
void branching_process(std::vector<basic_particle> &part_1,double proba_repro, double proba_death)
{
	...
	for(int j=0;j<size;j++)
	{
		...
		if (proba<proba_repro) //new particle at the same place, and the parent is still there. The new particle is added at the end of the vector of particles.
		{
...
int main()
{
	...
	std::ofstream f0,f1;
	double C; //Concentration of particles
	...
	dpow=0.25; //Increment between pow_min and pow_max (will determine the number of points we have for pcf=f(r/Delta))

	//Open the file in which we will have the x, y, parent of each particle
	f0.open("Spatial_distribution_particle_Fig1.txt");
	f1.open("pcf_particle_Fig1.txt"); //We compute the pcf but it is not necessary for Fig.1 graph
...
//		Part_table[j].pierrehumbert_flow(Utot, k, phi,theta, Lmax); //There is no advection for Fig.1
		if((t==100)||(t==tmax))
                {
                f0<< t <<";";
	...
	} //end j, i.e. the particle movement
	} //end t, i.e the whole simulation
	f0.close();

	//End of the simulation: we compute the pcf
	pow_i=pow_min;
	while (pow_i<pow_max)
	{
		printf("pow %lg\n",pow_i);
