...
\begin{subequations} 
\begin{align}
(k+1)\mu\int\mathcal{P}_{k+1}(\boldsymbol{x},\boldsymbol{y},\boldsymbol{X}_{k-2},\boldsymbol{z},t)d\boldsymbol{X}_{k-2}d\boldsymbol{z} & =(k+1)\mu\int\mathcal{P}_{k+1}(\boldsymbol{x},\boldsymbol{y},\boldsymbol{X}_{k-1},t)d\boldsymbol{X}_{k-1}\label{ck_death_1}\\
...
In cartesian coordinates, the derivation of the pair correlation function is:
...
We here prove that, using advection only, we have
...
We are using here the equations from Kraichnan (1974) \cite{kraichnan_convection_1974}. Let $\boldsymbol{r}(t)$
...
of $\boldsymbol{q}$. We have $Q=r^{d}G$. We know that (from Kraichnan
